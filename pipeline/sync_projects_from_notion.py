...
projects_mdx_dir = Path("../projects")

notion = Client(auth=os.getenv("NOTION_API_KEY", None))
portfolio_db_id = os.getenv("NOTION_PROJECTS_DB_ID", None)
front_path = {"images": "/images/projects/", "files": "/files/projects/"}
content_paths = {
    "page_id": ["id"],
    ...
    "stack": ["properties", "stack", "multi_select", ["name"]],
}
...
def read_existing_mdx_files():
    ...
    if not projects_mdx_dir.exists():
        return mdx_map
    for path in projects_mdx_dir.glob("*.mdx"):
...
def sync_records_to_mdx(records):
    projects_mdx_dir.mkdir(parents=True, exist_ok=True)
    ...
    for record in records:
        ...
        if old is None:
            path = projects_mdx_dir / f"{slug}.mdx"
        ...
        if old_slug != slug:
            old_path = old["path"]
            new_path = projects_mdx_dir / f"{slug}.mdx"
        ...
        image_dir = Path(f"../public/images/projects/{slug}")
        file_dir = Path(f"../public/files/projects/{slug}")
        deleted_images = [
            f
...
results = load_db_values(portfolio_db_id)
records = []
slugs = []
