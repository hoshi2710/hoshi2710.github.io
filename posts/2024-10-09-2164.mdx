---
page_id: 38aa954b-2063-81b4-8c61-c19b0b003878
title: BOJ - [Queue - 2164 카드2] - SILVER IV🥈
slug: 2024-10-09-2164
featured: false
description: https://www.acmicpc.net/problem/2164 이 문제에서 말하는 카드를 섞고 버리는 작업을 수행하기 위해서는
  큐(Queue) 형태의 자료 구조를 활용하면 된다. 이미 큐에 대해서는 예전 문제에서 설명하였으니 아래 링크를 참고.
category: 백준
tags:
- 백준
- Queue
...
- 백준_Silver
date: '2024-10-09'
content_hash: 3a8fadeda2982d0179aa92ba8a2d297c3917c0596eba7eb367a31ef30008e9e7
---
...
그다음 \$\$2\^0\$\$의 값을 별도의 변수(이하 A라 지칭) 에 더하기<br>(\$\$2\^0\$\$에서 0은 반복작업이 이루어진 횟수를 의미, 0부터 시작)
1. 나누기 전 값이 홀수라면 첫번째 작업에서 카드를 버리기까지만 하고 뒤로 보내는 작업은 수행 안한것 이므로 다음 작업에서는 작업을 거꾸로(역방향) 하는 것으로 기억. ( 짝수면 그대로 작업)
2. 이번에도 n을 나누는데 만약 위에서 작업을 거꾸로 하는 것으로 기록 되었다면 1번에서 나눈 값에 1을 더하고 다시 2로 나눈다. (정방향이면 1번과 동일한 작업 진행)
3. 2번과 동일한 조건기준으로 역방향, 정방향을 판단하고, 여기에 추가조건으로 3번 작업이 역방향인지 정방향인지 확인후 아래 처럼 결과를 도출(XOR 연산)
...
따라서 이 문제가 의도한 방법으로 푼것은 아니긴 하나,<br>효율 측면에서만 봤을때는 위에서 설명한 알고리즘이 상대적으로 더 효율적이었음을 확인할 수 있었다.
